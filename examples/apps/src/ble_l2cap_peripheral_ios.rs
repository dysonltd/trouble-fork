    ...
    C: Controller,
{
    ...
    let _ = join(runner.run(), async {
        loop {
            ...
            let l2cap_fut = l2cap_task::<_, _, L2CAP_MTU, PSM_CID>(&conn, &stack);
...
async fn l2cap_task<'a, C: Controller, P: PacketPool, const L2CAP_MTU: usize, const PSM: u16>(
    ...
    loop {
        match channel.receive(&stack, &mut buf).await {
            Ok(len) => {
                ...
                // echo received data
                match channel.send(&stack, rx_data).await {
                    Ok(_) => {
